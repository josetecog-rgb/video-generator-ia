...
// Polling para jobs async de fal.ai
async function getJobResult(requestId) {
  const response = await client.get(`/queue/requests/${requestId}/status`);
  return response.data;
}

module.exports = { generateImage, generateVideo, getJobResult };
